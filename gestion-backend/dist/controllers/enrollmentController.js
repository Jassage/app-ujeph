...
export const ensureSingleActiveEnrollment = async (studentId) => {
...
export const fixEnrollmentStatuses = async (req, res) => {
    const auditData = {
        ipAddress: req.ip || "unknown",
        userAgent: req.get("User-Agent") || "unknown",
        userId: req.userId || "unknown",
    ...
    try {
        ...
        for (const student of allStudents) {
            const beforeCount = (await prisma.enrollment.findMany({
                where: { studentId: student.id, status: "Active" },
            })).length;
            await ensureSingleActiveEnrollment(student.id);
            const afterCount = (await prisma.enrollment.findMany({
                where: { studentId: student.id, status: "Active" },
            })).length;
...
export const fixStudentEnrollmentStatus = async (req, res) => {
    const auditData = {
        ipAddress: req.ip || "unknown",
        userAgent: req.get("User-Agent") || "unknown",
        userId: req.userId || "unknown",
