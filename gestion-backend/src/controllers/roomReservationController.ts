...
export const getAllRoomReservations = async (req: Request, res: Response) => {
  try {
...
export const getRoomReservationById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const roomReservation = await prisma.roomReservation.findUnique({ where: { id } });
    if (!roomReservation) return res.status(404).json({ error: "RoomReservation non trouvé" });
...
export const createRoomReservation = async (req: Request, res: Response) => {
  try {
    const data = req.body;
...
export const updateRoomReservation = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const data = req.body;
...
export const deleteRoomReservation = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
