  ...
  try {
    ...
    if (!studentId || !academicYearId || !semester || !level) {
      return res.status(400).json({
        error:
          "Données manquantes: studentId, academicYearId, semester et level sont requis",
      });
    }
    ...
    const newTranscript = await prisma.transcript.create({
      data: {
        ...
        semester: semester === "all" ? "ANNUAL" : semester,
        level,
        documentType: documentType as any, // Conversion pour Prisma
  ...
  try {
    ...
    if (!transcript.pdfData) {
      ...
      const grades = await prisma.grade.findMany({
        where: {
          id: {
            in: transcript.transcriptGrades.map(
              (tg: { gradeId: any }) => tg.gradeId
            ),
          },
        },
        include: { ue: true },
    ...
    const fileName = `${transcript.documentType.toLowerCase()}-${transcript.student.lastName}-${
      transcript.student.firstName
    }-${transcript.academicYear.year}-${transcript.semester}.pdf`;
...
function generateBulletinPDF(doc: jsPDF, options: GeneratePDFOptions): Buffer {
  const { student, academicYear, semester, level, grades, statistics } =
    options;
