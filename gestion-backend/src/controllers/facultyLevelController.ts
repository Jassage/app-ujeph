...
export const getAllFacultyLevels = async (req: Request, res: Response) => {
  try {
...
export const getFacultyLevelById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const facultyLevel = await prisma.facultyLevel.findUnique({ where: { id } });
    if (!facultyLevel) return res.status(404).json({ error: "FacultyLevel non trouvé" });
...
export const createFacultyLevel = async (req: Request, res: Response) => {
  try {
    const data = req.body;
...
export const updateFacultyLevel = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const data = req.body;
...
export const deleteFacultyLevel = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
