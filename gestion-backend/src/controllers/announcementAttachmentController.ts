...
export const getAllAnnouncementAttachments = async (req: Request, res: Response) => {
  try {
...
export const getAnnouncementAttachmentById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const announcementAttachment = await prisma.announcementAttachment.findUnique({ where: { id } });
    if (!announcementAttachment) return res.status(404).json({ error: "AnnouncementAttachment non trouvé" });
...
export const createAnnouncementAttachment = async (req: Request, res: Response) => {
  try {
    const data = req.body;
...
export const updateAnnouncementAttachment = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const data = req.body;
...
export const deleteAnnouncementAttachment = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
