...
export const createFaculty = async (req: Request, res: Response) => {
  ...
  try {
    const {
      ...
      studyDuration,
      status = "Active",
      deanId, // Recevoir deanId depuis le body
    } = req.body;
    ...
    if (!name || !code) {
      await createAuditLog({
        ...
        entity: "Faculty",
        description:
          "Tentative de création de faculté - champs obligatoires manquants",
        status: "ERROR",
      });
    ...
    const levels = Array.from(
      { length: facultyData.studyDuration },
      (_, i) => ({
        level: `L${i + 1}`,
      })
    );
  ...
  try {
    ...
    if (user?.role === "Doyen") {
      await createAuditLog({
        ...
        entity: "Faculty",
        description:
          "Tentative de suppression de faculté - permission refusée pour le rôle Doyen",
        status: "ERROR",
      });
    ...
    if (faculty._count.assignments > 0) {
      ...
      return res.status(400).json({
        message:
          "Impossible de supprimer cette faculté car elle a des affectations de cours",
      });
    }
...
export const getFacultyStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createAuditLog({
      ...
      entity: "Faculty",
      description:
        "Erreur lors de la récupération des statistiques des facultés",
      status: "ERROR",
      errorMessage: error.message,
    });
...
export const getDeanFaculty = async (req: Request, res: Response) => {
  const auditData = {
    ipAddress: req.ip || "unknown",
    userAgent: req.get("User-Agent") || "unknown",
    userId: (req as any).userId || "unknown",
