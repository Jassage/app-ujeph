...
function shouldLogRequest(req: Request, statusCode: number): boolean {
  ...
  if (EXCLUDED_METHODS.includes(req.method)) return false;

  // Ne pas logger les paths exclus
  if (EXCLUDED_PATHS.some((path) => req.url.startsWith(path))) return false;

  // Logger seulement les erreurs importantes et les actions de modification
  return (
    ["POST", "PUT", "PATCH", "DELETE"].includes(req.method) || statusCode >= 400
  );
}
...
function getEntityIdFromUrl(url: string): string | undefined {
  const segments = url.split("/").filter((segment) => segment);
  if (segments.length > 0 && segments[0] === "api") {
